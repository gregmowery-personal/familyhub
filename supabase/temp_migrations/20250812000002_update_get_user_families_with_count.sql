-- Update get_user_families function to include member count
-- This migration adds member_count to the return type
...
BEGIN
  ...
  SELECT 
    ...
    fm.joined_at,
    (SELECT COUNT(*) FROM family_memberships fm2 
     WHERE fm2.family_id = f.id 
     AND fm2.status = 'active') as member_count
  FROM families f
...
-- Add comment
COMMENT ON FUNCTION get_user_families(UUID) IS 'Returns all active families a user belongs to with their role information and member count';